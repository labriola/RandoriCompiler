...
public class MetaDataUtils
{
    ...
    public static final Mode getMode(IDefinition definition)
    {
        ...
        String value = tag.getAttributeValue("mode");
        if (value == null)
            return Mode.PROTOTYPE;
    ...
    public static boolean isProtoExport(IDefinition definition)
    {
        ...
        if (tag != null)
        {
            ...
            if (exportValue == null && modeValue == null)
                return true;
            if (exportValue == null
                    && !modeValue.equals(VALUE_GLOBAL)
                    && !modeValue.equals("json"))
                return true;
    ...
    {
        IClassDefinition type = ExpressionUtils.getClassDefinition(definition);
        if (type == null)
            return false;
    ...
    public static String findJavaScriptCodeTag(IMetaTagNode node)
    {
        ...
        String reletivePath = node.getAttributeValue("file");
    ...
    public static String findJavaScriptCodeTag(IFunctionNode node)
    {
        ...
        IMetaTag tag = definition.getMetaTagByName("JavaScriptCode");
        if (tag == null)
            return null;

        String path = node.getSourcePath();
        File file = new File(path).getParentFile();
        String reletivePath = tag.getAttributeValue("file");
        File open = new File(file, reletivePath);
        String result = "";
        try
        {
    ...
    static final IMetaTag findTag(IDefinition definition, MetaData metaData)
    {
